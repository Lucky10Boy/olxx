...
export const registerComplete = (emailVerificationCode) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.post(`${process.env.REACT_APP_SERVER_API}/user/register/complete`, { emailVerificationCode });
...
export const registerNewUserPhoneNumber = (name, phoneNumber, password) => async (dispatch) => {
  dispatch({ type: REGISTER_REQUEST });

  try {
    const { data } = await axios.post(`${process.env.REACT_APP_SERVER_API}/user/register`, {
      name,
      phoneNumber,
      password,
    });
...
export const login = (email, password) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.post(`${process.env.REACT_APP_SERVER_API}/user/login`, {
      email,
      password,
    });
...
export const editPhoneNumber = (phoneNumber, oldPhoneNumber) => async (dispatch) => {
  dispatch({ type: EDIT_PHONE_NUMBER_REQUEST });

  try {
    const { data } = await axios.post(`${process.env.REACT_APP_SERVER_API}/user/edit/phonenumber`, {
      phoneNumber,
      oldPhoneNumber,
    });
    sendToastMsg(data.status, data.message);
