...
export const CategoryProducts = (props) => {
  ...
  useEffect(() => {
    props.getProductsByCategory(categoryId);
    console.log(props);
  }, []);
  return (
    <div style={{ height: '100vh', backgroundColor: '#f2f4f5' }}>
      ...
      <div className='row'>
        {props.productsByCategory ? (
          props.productsByCategory.map((p, i) => (
...
const mapStateToProps = (state) => {
  return { productsByCategory: state.product.productsByCategory };
};

export default connect(mapStateToProps, { getProductsByCategory })(CategoryProducts);
